	...
	while (true) {
		...
		if (action == TAG_ACTION) {
			...
			{ // Element löschen.
				...
				// Feedback senden.
				MPI_Send(&feedback, 1, MPI_INT, source, TAG_FEEDBACK, thread_comm);
				if (deleted)
			...
			{ // Element einfügen.
				...
				// Value empfangen.
				printf("CREATED BUFFER UND GETTING VALUE.\n");
				MPI_Recv(&valueArray, valueSize, MPI_CHAR, 0, TAG_VALUE, thread_comm, MPI_STATUS_IGNORE);
				printf("GOT VALUE.\n");
			...
			{ // Element holen.
				...
				if (value.empty()) {
					...
					valueSize = -1;
					MPI_Send(&valueSize, 1, MPI_INT, source, TAG_VALUE_SIZE, thread_comm);
				} else {
